...
const Certificate = React.forwardRef<HTMLDivElement, CertificateProps>(({ member, event, verificationUrl, onAssetsLoaded }, ref) => {
  ...
  return (
    <div ref={ref} className="w-full max-w-3xl mx-auto aspect-[12/8] relative rounded-lg overflow-hidden shadow-2xl border-4 border-primary/50 bg-background">
      {/* Use a standard img tag for better html2canvas compatibility */}
      <img
        src={certificateImage}
        alt={event.name || "Certificate background"}
        crossOrigin="anonymous" // Required for html2canvas to access the image from another domain
        onLoad={onAssetsLoaded} // Notify parent when image is loaded
        className="absolute inset-0 w-full h-full object-cover"
        data-ai-hint="certificate background"
      />
      <div className="absolute inset-0 bg-background/80 flex flex-col items-center justify-between p-2 sm:p-6 md:p-8 text-center text-foreground font-serif">
        ...
        <div className="flex flex-col items-center">
            ...
            <h1 className="text-xl sm:text-3xl md:text-4xl lg:text-5xl font-bold my-1 md:my-4 text-primary font-headline tracking-wide">
              {member.userName}
            </h1>
            <p className="text-xs sm:text-base md:text-lg">for their active role as a</p>
            <p className="text-base sm:text-xl md:text-2xl lg:text-3xl font-semibold my-1 md:my-2 text-accent-foreground bg-accent px-2 py-0.5 md:px-4 md:py-1 rounded-md">
              {member.role}
            </p>
            <p className="text-xs sm:text-base md:text-lg">in the</p>
            <h3 className="text-lg sm:text-2xl md:text-3xl lg:text-4xl font-semibold mt-1 md:mt-2 text-primary">
              {event.name}
            </h3>
        </div>
